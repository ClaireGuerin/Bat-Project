...
###----------BOXPLOTS----------###
	...
	for (j in 1:3){
		...
		Boxplot(ec_index~ec_ids, main=subplots[j], col=color_transparent30[1], border=color[1], cex.axis=1.5, ylab = index, xlab = "Individual ID", ylim=c(0,indlim), cex.lab=1.5, col.lab="gray30", cex.main=1.5, id.method="none") 
...
for (j in 1:3){
	...
	plot(ec_index_all~jitter(param_all, jitterStrength), main=subplots[j], cex.main=1.5, cex.axis=1.5, col=color_transparent30[1], ylab = index, 
		xlab = varName, ylim=c(0,maxy), cex.lab=1.5, col.lab="gray30", xaxt="n", pch=21, bg=color_transparent20[1]) 
	axis(1, at=param_for_means, las=1, labels=round(exp(param_for_means)))
